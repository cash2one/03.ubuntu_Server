...
class HdwanSpider(CrawlSpider):
    ...
    start_urls = ['http://www.hdwan.net/newpost']

    rules = (
        # 提取匹配 'category.php' (但不匹配 'subsection.php') 的链接并跟进链接(没有callback意味着follow默认为True)
        Rule(LinkExtractor(allow=('/\w*', ),deny=('/tag/.*','/wp-login.php.*' )),follow= True),
    ...
    def parse_item(self, response):
        ...
        item['cate']  = code(sel.xpath('//a[@itemprop="breadcrumb"]/text()').extract()[-1])
